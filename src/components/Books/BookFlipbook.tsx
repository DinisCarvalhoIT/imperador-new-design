...
import { PageFlip } from '@/lib/flipbook/StPageFlip-updated/src/PageFlip';
import { SizeType } from '@/lib/flipbook/StPageFlip-updated/src/Settings';
import type { IFlipSetting, IEventProps } from '@/lib/flipbook/settings';
...
const HTMLFlipBookForward = React.forwardRef(
    (props: IProps, ref: Ref<PageFlip | undefined>) => {
        ...
        useEffect(() => {
            ...
            if (pages.length > 0 && childRef.current.length > 0 && htmlElementRef.current) {
                removeHandlers();
