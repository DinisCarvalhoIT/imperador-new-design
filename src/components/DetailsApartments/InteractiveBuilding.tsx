...
export default function InteractiveBuilding({
  ...
  const [renderWidth, setRenderWidth] = React.useState<number>(initialWidth);
  const [renderHeight, setRenderHeight] = React.useState<number>(initialHeight);
  const [hovered, setHovered] = React.useState<HoverState | null>({ type: "T1", shapeIndex: null });
  ...
  // Efficient media query tracking without SSR issues
  React.useEffect(() => {
    ...
    if (mql.addEventListener) mql.addEventListener("change", handle as unknown as EventListener);
    else mql.addListener(handle as unknown as (this: MediaQueryList, ev: MediaQueryListEvent) => void);
    return () => {
      if (mql.removeEventListener) mql.removeEventListener("change", handle as unknown as EventListener);
      else mql.removeListener(handle as unknown as (this: MediaQueryList, ev: MediaQueryListEvent) => void);
    };
  }, []);

  // Throttled element size tracking
  React.useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      if (
        nextW > 0 &&
        nextH > 0 &&
        (Math.round(nextW) !== Math.round(renderWidth) || Math.round(nextH) !== Math.round(renderHeight))
      ) {
        setRenderWidth(nextW);
        setRenderHeight(nextH);
      }
    };
    const schedule = () => {
      if (rafId) return;
  ...
  function getScaledShapes(
    imageWidth: number,
    imageHeight: number,
    base: Record<ApartmentType, Rect[]>
  ...
  const SHAPES = React.useMemo(
    () =>
      getScaledShapes(
        renderWidth,
        renderHeight,
        isMobile ? BASE_SHAPES_MOBILE : BASE_SHAPES
      ),
    [renderWidth, renderHeight, isMobile]
  );
              ...
              <div className="flex-1 bg-[#0B1D26]/90 flex flex-col items-center justify-center relative z-10 px-8 py-4 overflow-y-auto">
                ...
                >
                  {isMobile ? (
                    ...
                    <div className="flex items-center justify-center gap-6 w-full relative px-12 sm:px-16 md:px-20 ">
